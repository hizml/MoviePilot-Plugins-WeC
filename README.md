...
| 功能 | 旧命令格式（已废弃） | 新命令格式（v2.4.3+） |
|------|---------------------|----------------------|
| 触发登录 | `#登录企业微信` | `/ww login` |
| 发送验证码 | `#123456` | `/ww 123456` |
...
**升级注意**：
- 旧命令格式已不再支持，请使用新命令格式
- 如已启用 ChatGPT 全局助手，新命令格式不会产生冲突
- 两个版本（WeWorkIP v2.5 和 WeWorkIPPW v2.4.3）均已更新  

## 登录流程
默认开启CookieCloud（简称CC），从MP官方的CC配置读取cookie
当插件检测到登录cookie失效时，会先从CC获取企业微信cookie尝试登录(防止顶掉其他地方的登录)
自动登录开关默认关闭，发送 `/ww login` 至MP应用则可以唤起一次登录操作。如果需要验证手机，把验证码按照格式 `/ww 123456` 发送到MP应用
