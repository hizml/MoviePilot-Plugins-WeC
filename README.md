...
## ⚠️ 重要更新（v2.6.2/v2.4.5+）

**命令格式已更新，完全符合 MoviePilot 官方规范！**

| 功能 | 旧命令格式（已废弃） | 当前命令格式（v2.6.2+） |
|------|---------------------|----------------------|
| 触发登录 | `#登录企业微信` → `/ww login` | `/ww_login` |
| 发送验证码 | `#123456` → `/ww 123456` | `/ww_code 123456` |

**变更原因**：
- ✅ 使用 `/` 前缀符合 MoviePilot 插件规范
- ✅ 避免 ChatGPT 插件冲突（ChatGPT 不会处理 `/` 开头的命令）
- ✅ 统一简洁的前缀风格（`/ww` = WeWork）
- ✅ **所有命令使用 PluginAction 正式注册，命令可见、有反馈、支持 UI 按钮**

**升级注意**：
- 旧命令格式不再支持，请使用新命令格式
- 所有命令都会立即返回执行确认消息
- 登录命令："正在启动登录流程..." 或 "缓存有效，无需登录"
- 验证码命令："✅ 已收到验证码：123456" 或 "❌ 验证码格式错误"
- 两个版本（WeWorkIP v2.6.2 和 WeWorkIPPW v2.4.5）均已更新  

## 登录流程
默认开启CookieCloud（简称CC），从MP官方的CC配置读取cookie
当插件检测到登录cookie失效时，会先从CC获取企业微信cookie尝试登录(防止顶掉其他地方的登录)
自动登录开关默认关闭，发送命令 `/ww_login` 至MP应用则可以唤起一次登录操作。如果需要验证手机，使用命令 `/ww_code 123456` 提交验证码
